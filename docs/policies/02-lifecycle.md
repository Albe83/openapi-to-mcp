...
4. Produce architecture artifacts (ADR in `docs/adr/` + diagrams in `docs/diagrams/`).
...
Use `.github/ISSUE_TEMPLATE/feature-or-bug-analysis.yml`.
...
Create one sub-issue per task using `.github/ISSUE_TEMPLATE/sub-task.yml`.
...
After implementation review, create one sub-issue per finding per `10-review-and-findings.md` and apply the same labeling rule.
