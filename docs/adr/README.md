...
7. Links to Class and Sequence diagram files in `docs/diagrams/`.
8. If data model changes: links to schema files in `docs/schemas/`.

## Process
1. Create/update ADR(s) before implementation.
2. Reference ADR(s) in sub-issues and PR.
3. If data model changes, update JSON Schema artifacts (YAML preferred) in `docs/schemas/`.
